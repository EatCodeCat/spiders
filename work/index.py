...
class TouZhu(threading.Thread):
    def __init__(self, keyword, gd_no_list, _id, complate_cb):
        threading.Thread.__init__(self)
        self.keyword = keyword
        self.gd_no_list = gd_no_list
        self.id = _id
        self.complate_cb = complate_cb

    def do_touzhu(self, item):
...
class Task(threading.Thread):  # The timer class is derived from the class threading.Thread
    ...
    def run(self):  # Overwrite run() method, put what you want the thread do here
        ...
        try:
            while not self.thread_stop:
                # print('wait task executing, Time:%s\n' % (datetime.datetime.now()))
                cursor = conn.execute('select * from task order by id desc')
                all = cursor.fetchall()
                for item in all:
                    if item[5] == '0' and self.eq_time(item[8], item[9], item[10]) and item[0] not in self.taskSet:
                        print("任务开始执行：", *item)
                        self.taskSet.add(item[0])
                        TouZhu(item[3], item[4], item[0], lambda id: self.taskSet.remove(id)).start()
...
def add_scheduler(name: str, key: str, idlist, h, m, s):
    # status 0 等待执行， 1 暂停, 2 停止,3 正在执行
